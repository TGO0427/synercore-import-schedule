...
/**
 * Add a diagonal watermark with the logged-in user's name to every page of a PDF document.
 * Deters sharing of exported documents.
 */
const addWatermark = (doc) => {
  const user = authUtils.getUser();
  const displayName = user?.full_name || user?.username || 'Unknown User';
  const watermarkText = `Exported by: ${displayName}`;
